...
# Resolve the feature documentation root directory (relative to repo root).
# Configured in .specify/init-options.json as "feature_root"; defaults to docs/features.
get_feature_root() {
    local repo_root="${1:-}"
    if [[ -z "$repo_root" ]]; then
    ...
    local feature_root="docs/features"
    local init_options="$repo_root/.specify/init-options.json"
    if [[ -f "$init_options" ]]; then
        if command -v jq >/dev/null 2>&1; then
            local configured
            configured=$(jq -r '.feature_root // empty' "$init_options" 2>/dev/null || true)
            if [[ -n "$configured" && "$configured" != "null" ]]; then
                feature_root="$configured"
            fi
        elif command -v python3 >/dev/null 2>&1; then
            local configured
            configured=$(python3 -c "import json; d=json.load(open('$init_options')); print(d.get('feature_root') or '')" 2>/dev/null || true)
