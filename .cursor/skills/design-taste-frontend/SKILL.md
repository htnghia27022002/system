...
description: Anti-slop frontend design skill for landing pages, portfolios, marketing pages, and visual redesigns. Use when the task is design direction, aesthetic quality, hero/section composition, or high-fidelity UI polish in React/Tailwind. The agent reads the brief, infers the right design direction, and ships interfaces that do not look templated. Real design systems when applicable, audit-first on redesigns, strict pre-flight check.
...
## Appendices (External Reference)

Install commands, canonical documentation links per design system, and the Apple Liquid Glass web-approximation skeleton live in `reference.md` (same folder). Read `reference.md` only when you need design-system specifics or starter snippets; keep this file as the working ruleset.
